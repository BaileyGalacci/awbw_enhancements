...
function makeProximityTile(rgbCode) {
    ...
    tile.background-color = rbgCode.toString(16); // .toUpperCase() ??
    return tile;
}
...
class MoveRangePreview {
    ...
    displayProximityTiles(proximityDict, tileFactory) {
        for (let {position, tileProx} in proximityDict) {
            //let position = positions[positionId];
            if (!this.isInBounds(position)) {
                continue;
            }
            ...
            //let tileProx = proximityMap[position];
    ...
    getShortestDistanceToTiles(unit) {
        ...
        let visited = {};
        visited[startId] = 0;
        let queue = [{coord: unit.coords, totalCost: startingMove}];
        let nextQueue = [];
        while (queue.length > 0) {
            for (let {coord, totalCost} of queue) {
                ...
                for (let neighborId in neighbors) {
                    ...
                    if ((totalCost % movementRange) + terrainCost > movementRange) {
                        newCost = totalCost - (curCost % movementRange) + movementRange + terrainCost;
                    } else {
                        newCost = totalCost + terrainCost;
                    }
                    
                    if (newCost < visited[neighborId]) {
    ...
    findTurnsToTiles(unit) {
        ...
        distances = getShortestDistancesToTiles(unit);
        for (const [key, value] of Object.entries(distances)) {
          myMap[key] = parseInt(myMap[key]/movementRange);
        }
